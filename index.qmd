...
Researchers in precision livestock technology often face the same pattern:
...
- Quick refresh of SQL and R basics
...
Downloading the repository from GitHub will install the example tables in a local `.duckdb` file. You can also create your own database from CSV files by running the  `00_setup_duckdb.R` with example tables for animals, GPS fixes, and weights.
...
We will demonstrate how to open the `.duckdb` file in DBeaver, run queries, and visualize results.
...
- how do we summarize high-frequency observations (accelerometers) without losing provenance?
...
Which of these topics would you like to know more about? What would you like to see in a follow-up workshop?
